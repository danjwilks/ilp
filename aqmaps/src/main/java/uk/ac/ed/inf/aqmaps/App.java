...
public class App {
	...
	public static FeatureCollection getBuildings() throws IOException, InterruptedException {
		...
		String buildingsJSON = getJSON("http://localhost/buildings/no-fly-zones.geojson");
		return FeatureCollection.fromJson(buildingsJSON);		
	}
	
	public static List<Station> getStations(String day, String month, String year) {
		
		String stationsJSON = getJSON("http://localhost/maps/" + year + "/" + month + "/" + day + "/air-quality-data.json");
		List<Station> stations = buildStations(stationsJSON);
		return stations;
	}
	
	public static List<Station> buildStations(String stationsJSON) {
		
		if (stationsJSON.equals("")) {
			return new ArrayList<>();
		}
		
		Type listType = new TypeToken<ArrayList<Station>>() {}.getType();
    	List<Station> stations = new Gson().fromJson(stationsJSON, listType);
    	return stations;
	}
...
//    	args = 15 06 2021 55.9444 -3.1878 5678 80
    	...
    	String month = args[1];
    	String year = args[2];
    	double xStartCoord = Double.parseDouble(args[3]);
    	double yStartCoord = Double.parseDouble(args[4]);
    	int randomSeed = Integer.parseInt(args[5]);
    	int portNumber = Integer.parseInt(args[6]);
    	

    	var buildings = getBuildings();
    	var stations = getStations(day, month, year);
    	double[] startLocation = {xStartCoord, yStartCoord};
